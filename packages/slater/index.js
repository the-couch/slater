...
function logStats(stats, opts = {}) {
  log.info(
    ...
    `in ${stats.duration}s\n${stats.assets.reduce((_, asset, i) => {
      ...
      return (_ += `  > ${log.colors.gray(asset.name)} ${size}${
        i !== stats.assets.length - 1 ? `\n` : ""
      }`);
    }, "")}`
  );
}
      ...
      function syncFile({ filename, src, dest }) {
        ...
        return theme
          ...
          .catch(({ errors, key }) => {
            log.error(
              `syncing ${key} failed - ${
                errors.asset ? errors.asset.join("  ") : errors
              }`
            );
          });
      }
        ...
        chokidar
          ...
          .on(
            ...
            (file) =>
              file &&
              syncFile(
                formatFile(fixPathSeparators(file), config.in, config.out)
              )
          )
          .on(
            "change",
